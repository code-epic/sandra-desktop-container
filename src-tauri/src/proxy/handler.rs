...
use super::remote::{is_app_proxy_required, get_active_connection, proxy_to_remote};
...
pub fn handle_request(app_handle: &AppHandle, request: &Request<Vec<u8>>) -> Response<Vec<u8>> {
    ...
    println!("🔍 Req: {} | Referer: {:?}", path, request.headers().get("referer"));

    // 0. Caso Especial: Proxy para URLs Externas
            ...
            if let Ok(base) = Url::parse(&target_url) {
                ...
                if let Ok(remote_full) = base.join(final_path) {
                    ...
                    match proxy_arbitrary_url(app_handle, &app_id, &remote_str, Some(base_href)) {
                        Ok(resp) => return resp,
                        Err(e) => return create_error_response(502, &format!("Proxy Error: {}", e)),
                    }
                }
            }
        }
    ...
    if let Some(referer) = request.headers().get("referer").and_then(|v| v.to_str().ok()) {
        if referer.contains("/external-proxy") {
            if let Some(target_start) = referer.find("target=") {
                ...
                let decoded_target = urlencoding::decode(clean_encoded).unwrap_or(std::borrow::Cow::Borrowed(clean_encoded));

                // Extract app_id from referer
                let app_id = referer.split("/external-proxy/").nth(1).and_then(|s| s.split('?').next()).unwrap_or("unknown");
    ...
    if path.contains("v1") {
        ...
        let referer_opt = request.headers().get("referer").and_then(|v| v.to_str().ok());
        let mut should_proxy = false;
        let mut app_id_context = String::new();

        if let Some(referer) = referer_opt {
            let after_scheme_opt = referer.strip_prefix("sandra-app://127.0.0.1/").or_else(|| referer.strip_prefix("sandra-app://localhost/"));
            if let Some(after_scheme) = after_scheme_opt {
                let app_id_candidates = if after_scheme.starts_with("external-proxy/") {
                    ...
                    after_scheme.split_once('/').map(|(id, _)| id).unwrap_or(after_scheme)
                };
        ...
        if should_proxy {
            if let Some(active_conn) = get_active_connection(app_handle) {
                match proxy_to_remote(app_handle, &app_id_context, active_conn, request) {
                    ...
                    Err(e) => {
                        return create_error_response(502, format!("Proxy Error (Remote Unreachable): {}", e).as_str());
                    }
                }
            }
        }
