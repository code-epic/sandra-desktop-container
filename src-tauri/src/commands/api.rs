...
fn get_auth_headers(hash: &str, temp_auth_token: Option<String>) -> Result<reqwest::header::HeaderMap, String> {
    ...
    // Crypto key consists of FIRST 32 digits of hash
    ...
    // Obtenemos Device-Context Cifrado
...
pub async fn api_post_raw_request(
    ...
    let headers = get_auth_headers(&hash, temp_auth_token)?;
...
pub async fn api_post_request(
    ...
    let res = api_post_raw_request(state, ip, port, endpoint, payload, hash, temp_auth_token).await?;
...
pub async fn api_get_raw_request(
    state: tauri::State<'_, DbState>,
    ip: String,
    port: u16,
    endpoint: String,
